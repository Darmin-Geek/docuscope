...
type DeleteFileModalProps = {
  // The filename being deleted, shown for confirmation.
  filename: string;
  onCancel: () => void;
  onConfirm: () => Promise<void>;
};

// Confirmation dialog for soft-deleting a file (issue #100). Mirrors
// DeleteInformationModal. The file must be checked out by the current user (the
// trash button that opens this is disabled otherwise), so this only confirms.
    ...
    >
      ...
      >
        ...
        <p className="mb-6 text-sm text-zinc-500 dark:text-zinc-400">
          {`“${filename || "Untitled"}” will be removed from the project. An administrator can recover it if needed.`}
        </p>
